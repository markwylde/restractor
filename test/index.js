...
const createFakeRestServer = require('./helpers/createFakeRestServer');
const restractor = require('../');

test('backup', async t => {
  t.plan(1);

  const fakeServer = await createFakeRestServer();

  const notes = await restractor.extract(`${fakeServer.baseUrl}/notes`);
...
test('backup - with pagination', async t => {
  t.plan(1);

  const fakeServer = await createFakeRestServer();

  const notes = await restractor.extract(`${fakeServer.baseUrl}/notes?page={page}`);
